...
export function interpretCommand(text, lang = 'ko') {
  ...
  // ③ Describe the current graphic (also catches audio-intent phrasing,
  //    which now yields a screen-reader text summary instead of a sound).
  if (/설명|묘사|describe|alt\s*text|뭐가\s*있|무엇|소리로?\s*(들|확인|듣)|음향|들려/i.test(s)) {
...
export const QUICK_COMMANDS = [
  ...
  { icon: 'noise',    text: { ko: '점이 너무 많아',        en: 'Too many dots' } },
  { icon: 'invert',   text: { ko: '밝고 어두움 반전',      en: 'Invert' } },

  { group: { ko: '도형 만들기', en: 'Create shapes' },
    icon: 'heart',    text: { ko: '하트 그려줘',           en: 'Draw a heart' } },
  { icon: 'star',     text: { ko: '별 그려줘',             en: 'Draw a star' } },
  { icon: 'butterfly',text: { ko: '나비 그려줘',           en: 'Draw a butterfly' } },
  { icon: 'globe',    text: { ko: '지구 그려줘',           en: 'Draw the earth' } },
  { icon: 'border',   text: { ko: '테두리 그려줘',         en: 'Draw a border' } },
  { icon: 'grid',     text: { ko: '격자 그려줘',           en: 'Draw a grid' } },

  { group: { ko: '수식', en: 'Math' },
    icon: 'wave',     text: { ko: 'y = sin(x)',           en: 'y = sin(x)' } },
  { icon: 'wave',     text: { ko: 'y = x * x - 4',       en: 'y = x² − 4' } },
  { icon: 'wave',     text: { ko: 'y = cos(x)',           en: 'y = cos(x)' } },

  { group: { ko: '텍스트', en: 'Text' },
